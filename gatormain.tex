...
This chapter focuses on real-time 3D rendering on GPUs, where correctness hazards in linear algebra code are particularly pervasive.
...
Figures~\ref{fig:bunnygood} and~\ref{fig:bunnygood} shows an example: a coordinate system handling bug yields incorrect visual output that would be difficult to catch with testing.
...
This chapter advocates for making geometric spaces manifest in programs themselves via a type system.
...
The core contribution of this chapter is that all three components of geometry types are necessary.  
...
We formalize a geometry type system and show that erasing these types preserves a (straightforward) property of producing a well--typed term.
...
we implement \emph{diffuse lighting}, a component of the classic Phong lighting model~\cite{phong}.\footnote{Appendix~\ref{app:llphong} gives a complete GLSL implementation of the Phong model.}
...
To expand on condition (3); canonical functions may take in \emph{canonical arguments}, which are variables labeled with the \code{canon} keyword.  The most familiar example of this use is defining matrix---vector multiplication to be canonical; the matrix itself must be included and must be a canonical matrix:
%
\begin{lstlisting}
with frame(3) target:
...
%
The transformation system should be \emph{coherent} (a term borrowed from language used for type coercions~\cite{coercion}):
...
Because it is a purely static system, Gator does not enforce coherence.
%
However, coherence motivates Gator's requirement that canonical transformations preserve dimensionality (see Section~\ref{sec:canon}).
%
Without this condition, we have found it is easy to accidentally violate coherence with non-invertible functions and result in an ambiguous transformation graph for \code{in} expressions.

Our construction of canonical functions and automatic transformations is similar to constructions provided by C\# and C++'s type coercion, which are themselves examples of general type-theoretic coercion functions.  Indeed, our intuition related to coherence can be summarized as what is (informally) needed to match the property needed for coherence given in~\cite{coercion}: "translations of any two derivations of the same typing judgement are equated in the target calculus".  The slightly different practicalities used by our \code{in} expression implementation will be discussed briefly in Section~\ref{sec:rw}.  
...
In this section, we formalize a core of Gator to provide details of typechecking, without proving type soundness.
...
Proving soundness of the Gator type system would be interesting future work but is out of scope for this chapter.
Concretely, to show Gator type soundness, we would need to extend our formalism with a precise definition of a geometry bug and how our type system would formally rule out such a bug.
...
The types in this core language consist of \code{unit} and a partial order with $\top_p$ and $\bot_p$ over each primitive type $p$.
...
Gator requires a partial order, along with a $\top_p$ and $\bot_p$, for each primitive type; custom types on each partial order introduce new subtyping relations.
...
Our reflection case study, shown in Figure~\ref{fig:shaders-1}, renders an object that reflects the dynamic scene around it, creating a ``mirrored'' appearance.
...
Shadow mapping is a technique to simulate the shadows cast by 3D objects when illuminated by a point light source.  Our case study, shown in Figure~\ref{fig:shaders-2}, renders several objects that cast shadows on each other and a single ``floor'' surface.  The non-shadow coloring is simulated through Phong lighting as previously discussed.
...
Our texture case study renders a face mesh with a single texture (shown in Figure~\ref{fig:shaders-1}).
